...
function salaryConvertor(array) {
    array.map((element) => {
        return Number(element.salary.slice(1));
    });
}
...
function salarySum(array) {
    return array.map((element) => {
        return Number(element.salary);
    })
        .slice(1)
...
function salarySumEachCountry(array) {
    let countrySalarySum = {};
    array.forEach(element => {
        if (countrySalarySum[element.location] == undefined) {
            countrySalarySum[element.location] = Number(element.salary.slice(1));
        } else {
            countrySalarySum[element.location] += Number(element.salary.slice(1));
        }
    })
...
// console.log(salarySumEachCountry(array));

// Question-6

function averageSalaryEachCountry(array) {
    let averageSalary = {};
    array.forEach((element) => {
        if (averageSalary[element.location] == undefined) {
            averageSalary[element.location] = {
                salary: Number(element.salary.slice(1)),
                personCount: 1,
                average: Number(element.salary.slice(1)) / 1
            };
        } else {
            averageSalary[element.location].salary += +element.salary.slice(1);
            averageSalary[element.location].personCount += 1;
            averageSalary[element.location].average = ((averageSalary[element.location].salary) / (averageSalary[element.location].personCount))
                .toFixed(2);
        }
    })
...
// console.log(averageSalaryEachCountry(array));