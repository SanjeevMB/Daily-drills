...
/* 
    ...
    2. Convert all the
     salary values into proper numbers instead of strings 
    3. Assume that each
     salary amount is a factor of 10000 and correct it but add it as a new key (corrected_salary or something)
    4. Find the sum of all salaries 
    5. Find the sum of all salaries based on country using only HOF method
    6. Find the average
     salary of based on country using only HOF method
...
function correctedSalary(array) {
    ...
    arrayClone.map((element) => {
        ...
        element.corrected_salary = Number(element
            .salary.slice(1)) * 10000;
...
function salarySumEachCountry(array) {
    ...
    array.forEach(element => {
        if (countrySalarySum[element
            .location] == undefined) {
            countrySalarySum[element
                .location] = Number(element
                    .salary.slice(1));
        } else {
            countrySalarySum[element
                .location] += Number(element
...
function averageSalaryEachCountry(array) {
    let averageSalary = {};
    array.forEach((element) => {
        if (averageSalary[element
            .location] == undefined) {
            averageSalary[element
                .location] = {

                salary: Number(element
                    .salary.slice(1)),
                personCount: 1,
            ...
            averageSalary[element
                ...
                .salary += +element
                    .salary.slice(1);
            averageSalary[element
                .location].personCount += 1;
            averageSalary[element
                .location].average = ((averageSalary[element
                    .location]
                    .salary) / (averageSalary[element
